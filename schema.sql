...
CREATE VIEW IF NOT EXISTS inbound_stock AS
SELECT
  i.id AS item_id,
  i.sku,
  i.name,
  COALESCE(SUM(sol.quantity), 0) AS inbound_quantity,
  MIN(so.expected_at) AS next_expected_at
FROM items i
LEFT JOIN supplier_order_lines sol
  ON sol.item_id = i.id AND sol.line_status IN ('ordered', 'shipped', 'partial')
LEFT JOIN supplier_orders so
  ON so.id = sol.supplier_order_id AND so.status IN ('ordered', 'shipped', 'partial')
GROUP BY i.id;

CREATE VIEW IF NOT EXISTS item_stock AS
SELECT
  i.id AS item_id,
  i.sku,
  i.name,
  ...
  COALESCE((
    ...
    FROM reservations r
    WHERE r.item_id = i.id AND r.status = 'reserved'
  ), 0) AS reserved,
  COALESCE(SUM(sm.quantity_delta), 0) - COALESCE((
    SELECT SUM(r.quantity)
    FROM reservations r
    WHERE r.item_id = i.id AND r.status = 'reserved'
  ), 0) AS available
